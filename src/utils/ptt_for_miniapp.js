...
const setSid = () =>
  setLocalData(
    "ssid",
    `s${(Math.round(49704631 * Math.abs(Math.random() - 1)) *
...
const getRouteLine = () => {
  ...
  if (!str || str.split("_").length <= 7) {
    ...
    try {
      pageType =
        PTT.setSite.pageType ||
        getPagePath()
          .split("/")
          .pop(); //如果没有设置SetSite.pageTpe，则获取route最后一个单词
    } catch (e) {}
  ...
  init: () => {
    ...
    Vue.mixin({
      ...
      onShow() {
        ...
        PTT.setSite.pageType = getPagePath()
          .split("/")
          .pop(); //onLoad的时候把setSite的pageType值重置
        PTT.setSite.pageName = "无"; //onLoad的时候把setSite的pageName值重置
        // API.setSite();
        // _pageOS && _pageOS.apply(this, arguments);
