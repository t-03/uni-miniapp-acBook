...
//全局的小程序业务属性配置
const setSite = {
  dm: 'dg.qq.com',             //*小程序所属的业务域名，重返帝国域名为dg.qq.com
...
//返回设备环境是手Q小程序还是微信小程序
const getPlat = 'undefined' != typeof qq ? 'dcv_qqapp' : 'undefined' != typeof wx ? 'dcv_wxapp' : 'dcv_none'
...
//设置sid
const setSid = () => setLocalData('ssid', `s${Math.round(49704631 * Math.abs(Math.random() - 1)) * (new Date).getMilliseconds() % 1E8}`)
...
//获取硬件信息pf
const getHardWare = () => {
  let SI = systemInfo, arr = [], d = 'none';
  arr.push("mo=" + (SI.hasOwnProperty("model") ? /[\w|\s]*/.exec(SI["model"])[0] : d))
  arr.push("pr=" + (SI.hasOwnProperty("pixelRatio") ? SI["pixelRatio"] : d));
  arr.push("vsize=" + (SI.hasOwnProperty("windowWidth") ? SI["windowWidth"] + "x" + SI["windowHeight"] : d));
  ...
  arr.push("ssize=" + (SI.hasOwnProperty("screenWidth") ? SI["screenWidth"] + "x" + SI["screenHeight"] : d));
...
const getPVHottag = () => {
  let arr = [];
  let SI = systemInfo, d = 'none';
  let br = 'undefined' != typeof qq ? 'qqapp' : 'undefined' != typeof wx ? 'wxapp' : d;          //浏览器信息和系统信息
  br += SI.hasOwnProperty("version") && SI["version"] || d;
  let viewsize = SI.hasOwnProperty("windowWidth") ? SI["windowWidth"] + "x" + SI["windowHeight"] : d;
  let devicesize = SI.hasOwnProperty("screenWidth") ? SI["screenWidth"] + "x" + SI["screenHeight"] : d;
  arr.push(weekloop());
...
const weekloop = () => {
  ...
  var nowWeek = Math.ceil(Math.ceil((new Date(y, m, d) - new Date(y, 0, 1) + _d) / 1000 / 60 / 60 / 24) / 7);		//得到当前日期是本年第几周
  var weekloop = getLocalData("weekloop");
  if (weekloop && weekloop.split("-")[3] == nowWeek) {								      //如果当前的周已经上报过了，则返回
    return "";
  }
  var stayArr = weekloop ? weekloop.split("-") : ["0", "0", "0", "0"];			//每周回访的周数链，比如今年第30周，31周，33周曾访问过，那就是[30,31,0,33]
  var L = 0;																														    //如果隔周回访，需要在中间添加几个0
  if (nowWeek > stayArr[3]) {	                                              //没有经历跨年的常规周,计算需要添加几个0
    L = nowWeek - stayArr[3] - 1;
  } else {									                                                //跨年的周,计算需要添加几个0
    var totalWeek = 53;																									  	//获取去年有几周
    if ((new Date(y - 1, 1, 29)).getDate() == 29 && new Date(y - 1, 0, 1).getDay() == 6) { 		//去年是否闰年且1月1日是否是周六；如果周一是一周的第一天，则是.getDay()==6修改为==0
      totalWeek = 54;
    }
...
const getRouteLine = () => {
  ...
  if (!str || str.split("_").length <= 7) {                                          //历史路径只记录8个
    try {
...
const PTT = {
  ...
  pvid: () => {
    ...
    if (!__pvid) {
      PTT.reserved1 = "new";
      __pvid = Math.round(49704631 * Math.abs(Math.random() - 1)) * (new Date).getMilliseconds() % 1E8;
  ...
  osact: (setSite) => {
    var tempStr = 0,
      osArr = ["0", "m", "pc", "ingame"];
    if (setSite.hasOwnProperty("osact")) {		//如果设置了osact
      tempStr = (setSite.osact + "").trimAll();
      if (osArr.indexOf(tempStr) == -1) {		//如果osact不符合取值范围
        console.warn("您设置的osact值超出范围，请在" + osArr.join(",") + "中选择一个,否则将以0代替");
        tempStr = 0;
      }
    }
    return tempStr;
  ...
  //返回官网体系内容模块名称
  project: (setSite) => {
    const projectList = ["base", "ingame", "doujin", "match", "story", "history", "fans", "coming", "bbs", "mct", "skin", "other", "hero", "commerce", "version", "brand", "ip", "community", "netbar", "important", "app"];
    if (!setSite.hasOwnProperty("project")) {
      if (setSite.hasOwnProperty("siteType") && setSite.siteType.toLowerCase() == "os") {
        return "base";
      } else {
        return "other";
      }
    ...
    } else { console.error("PTT组件提示：您设置的project参数值有误，目前只允许设置值为:" + projectList.join(",") + "请检查"); return "other"; }
  },
  ...
  init: () => {
    ...
    API.setSite = function(json){
      ...
      for(let key in json){
        if(key.toLocaleLowerCase() === "pagetype")json["pageType"] = json[key];
        if(key.toLocaleLowerCase() === "pagename")json["pageName"] = json[key];
      }
  ...
  getBaseData: () => {
    ...
    PTT_CONFIG.launchOptions =
      ...
      || {};
    let refer = "scene" in PTT_CONFIG.launchOptions ? PTT_CONFIG.launchOptions.scene : "0",
  ...
  PTTSendClick: (btnType, btnName, cnName) => {
    if (btnType === undefined || btnType === null) {
      throw new TypeError(`事件上报需要配置参数如:PTTSendClick("btn","name","中文名")`);
    }
    ...
    if (reservedWords.indexOf(btnType.toLowerCase()) != -1) {	//如果冲突了
      console.warn("由于您的按钮类型与MOSSO系统保留类型雷同，组件将您的按钮类型：\"" + btnType + "\"，重新命名为：\"" + btnType + "clash\"");
      btnType += "clash";
    }
  ...
  __PTTSendClick: (btnType, btnName, cnName, action) => {
    ...
    btnType = btnType == "" || typeof (btnType) == "undefined" ? "" : btnType.toString().replace(/_|\./g, "*-*");	//判断传入参数1是否为""或者没有传
    btnName = btnName == "" || typeof (btnName) == "undefined" ? "" : "_" + btnName.toString().replace(/_|\./g, "*-*");
    cnName = typeof (cnName) == "undefined" ? "" : "_" + cnName.toString().replace(/_|\./g, "*-*");
    ...
    API.request({
      url: dataArr.join("&"),
      success: () => setSite.debug && console.log("按钮/事件已成功上报", btnStr)
    })
  },
